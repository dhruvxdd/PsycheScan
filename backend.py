...
def data_cleaning():
    ...
    for column in to_drop:
        if column in df.columns :
            df.drop(column, axis=1, inplace=True) #inplace true ka matlab hai ke changes direct data frame mei honge naake kese new data frame mei (permanent change)
        else:
            print(f"Column '{column}' does not exist in the DataFrame.")

    # Apply the clean_text function to the 'post_text' column
    df['post_text'] = df['post_text'].apply(clean_text) #apply is a function ofpandas 
